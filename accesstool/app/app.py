...
QUERY = 'select mean("pm25") from "airquality" where time > now() - 24h group by time(2m) fill(none)'
MEDIAN = 'select mean("pm25") from "airquality" where time > now() - 24h group by "name"'
...
def download(reqfile):
    ...
    return send_file(
        requested_file, 
        mimetype='text/plain', 
        attachment_filename=requested_file, 
        as_attachment=True
    )
