...
class Bot:
    ...
    def __get_whitelist(self):
        try:
            ...
            logging.error("Couldn't get whitelist")
            self.apisession = r.Session()
    ...
    def __validate_user(self, username, chat_id):
        ...
        try:
            ...
            logging.debug(f"VALIDATION returned status code {validation.status_code}")
    ...
    def __get_updates(self):
        ...
        logging.debug("Getting updates from telegram")
        ...
        try:
            ...
            logging.debug("Incomplete updates gotten, skipping...")
            return

        except HTTPError as e:
            logging.critical("Reinitiating bot connection after HTTPError")
            self.s = r.Session()
            return

        except ConnectionError as e:
            logging.critical("Reinitiating bot connection after ConnError")
            self.s = r.Session()
            return

        except Timeout as e:
            logging.critical("Reinitiating bot connection after Timeout")
            self.s = r.Session()
            return

        except Exception as e:
            logging.critical("Reinitiating bot connection after exception")
            self.s = r.Session()
            return

        logging.debug("Updates gotten")
        ...
        logging.debug("Processing incoming messages")
        for msg in up['result']:
            logging.debug(f"Processing message {msg}")
            logging.debug("Parsing updates")
            self.__parse_msg(msg)
            logging.debug("Done parsing updates")

            _ofst = msg['update_id'] + 1

        logging.debug("Done Processing incoming messages")
